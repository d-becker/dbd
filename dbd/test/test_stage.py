...
from stage import Stage, StageException, StageExecutor
...
class TestStageExecutor(unittest.TestCase):
    ...
    def test_execute_in_order_correct_order(self) -> None:
        ...
        stages = [ListAppenderStage(i, append_list) for i in range(TestStageExecutor.LIST_LENGTH)]

        stage_executor = StageExecutor(stages)
        stage_executor.execute_in_order()

        expected_list = [i for i in range(TestStageExecutor.LIST_LENGTH)]
    ...
    def test_execute_in_order_unmet_precondition_raises(self) -> None:
        ...
        stages = [PreconditionCheckingListAppenderStage(i, append_list)
                  for i in reversed(range(TestStageExecutor.LIST_LENGTH))]
    ...
    def test_execute_needed_correct_stages_executed(self) -> None:
        append_list = [i for i in range(int(TestStageExecutor.LIST_LENGTH / 2))]

        stages = [PreconditionCheckingListAppenderStage(i, append_list)
                  for i in range(TestStageExecutor.LIST_LENGTH)]
    ...
    def test_execute_needed_no_precondition_is_met(self) -> None:
        append_list: List[int] = []
        stages = [PreconditionCheckingListAppenderStage(i, append_list)
                  for i in range(1, TestStageExecutor.LIST_LENGTH + 1)]
